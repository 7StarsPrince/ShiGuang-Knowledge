...
export async function POST(req: NextRequest) {
  try {
    ...
    const info = db.prepare(
      `INSERT INTO articles (title, source_name, source_url, author, summary, content, cover_image, published_at, topic_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(title, source_name || null, source_url || null, author || null, summary || null, content || null, cover_image || null, published_at || null, topic_id || null);
